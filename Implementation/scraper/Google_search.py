...
def call_google_search_save(queries, output_path="search_results_raw.ndjson", results_per_query=10):
    ...
    all_results = []  # Collect all results first, then deduplicate
    ...
    for q in queries:
        ...
        for item in items:
            ...
            # Normalize URL for deduplication (remove trailing slash, lowercase)
            normalized_url = result_url.rstrip('/').lower()
            ...
            row = {
                ...
                "url": result_url,  # Keep original URL format
    ...
    # Write all deduplicated results to file
    with open(output_path, "w", encoding="utf-8") as f_out:
        for row in all_results:
            f_out.write(json.dumps(row, ensure_ascii=False) + "\n")

    print(f"Saved {len(all_results)} unique search results to {output_path}")
